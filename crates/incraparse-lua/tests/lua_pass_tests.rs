...
use incraparse::{CancelToken, ParseTree, SerialExecutor, Span, Status};
use incraparse_lsp::Language;
use incraparse_lua::{LuaCtx, LuaLanguage};
use mlua::Lua;
...
fn outcome_protocol_expands() {
    ...
    assert_eq!(tree.status(tree.root()), incraparse::Status::Expanded);
...
fn deep_equality_gates_reuse() {
    ...
    tree.edit(incraparse::Edit::replace(19, 20, 20));
