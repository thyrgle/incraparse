//! Define an incraparse language server in **pure Lua**.
//!
...
//!
//! ```text
//! incraparse-lua-server ~/.config/minilang/lang.lua
//! ```
//!
//! # Semantics
//!
...
fn status_name(status: incraparse::Status) -> &'static str {
    match status {
        incraparse::Status::Unparsed => "unparsed",
        incraparse::Status::Expanded => "expanded",
        incraparse::Status::Done => "done",
        incraparse::Status::Failed => "failed",
    }
}
        ...
        match result {
            ...
            Value::Table(outcome) => {
                ...
                let Value::Table(array) = expand else {
                    eprintln!(
                        "incraparse-lua: pass #{} returned a table without `expand`; treating as failed",
                        self.index
                    );
            ...
            other => {
                eprintln!(
                    "incraparse-lua: pass #{} returned {}; treating as failed",
                    self.index,
                    other.type_name()
                );
                Ok(Outcome::Failed)
...
impl Pass for LuaPass {
    ...
    fn parse(&self, source: &str, span: Span, ctx: &LuaCtx) -> Outcome<LuaCtx> {
        match self.try_parse(source, span, ctx) {
            Ok(outcome) => outcome,
            Err(err) => {
                eprintln!("incraparse-lua: pass #{} error: {err}", self.index);
                Outcome::Failed
            }
        }
    }
...
/// [`incraparse_lsp::serve`](incraparse_lsp::serve):
///
/// ```no_run
/// fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
///     let language = incraparse_lua::LuaLanguage::from_path("minilang.lua")?;
...
impl Language<LuaCtx> for LuaLanguage {
    ...
    fn symbols(&self, doc: &Document<LuaCtx>) -> Vec<DocumentSymbol> {
        ...
        let result: Value = match function.call(nodes) {
            Ok(v) => v,
            Err(err) => {
                eprintln!("incraparse-lua: symbols error: {err}");
                return Vec::new();
            }
        };
