//! End-to-end smoke test: spawns the `increparse-lua-server` binary with the
...
fn spawn_server() -> (Child, ChildStdin, Receiver<String>) {
    let path = env!("CARGO_BIN_EXE_increparse-lua-server");
    ...
    let mut child = Command::new(path)
        ...
        .spawn()
        .expect("failed to spawn increparse-lua-server");
