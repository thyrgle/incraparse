...
use increparse_lsp::{serve_on, Documents, Language};
...
impl<const SYMBOLS: bool> Language<Ctx> for TestLang<SYMBOLS> {
    ...
    fn diagnostic(
        &self,
        doc: &increparse_lsp::Document<Ctx>,
        node: increparse_lsp::FailedNode<'_, Ctx>,
    ) -> Option<Diagnostic> {
    ...
    fn symbols(&self, doc: &increparse_lsp::Document<Ctx>) -> Vec<DocumentSymbol> {
